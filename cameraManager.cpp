...
void cameraManager::rectangle(HDC hdc, RECT mrec)
{
	RECT rect = { getRelativeX(mrec.left), getRelativeY(mrec.top), getRelativeX(mrec.right), getRelativeY(mrec.bottom) };
	Rectangle(hdc, rect);
}
...
void cameraManager::alphaFrameRender(HDC hdc, image * img, float destX, float destY, int frameX, int frameY, BYTE alpha)
{
	float newX = getRelativeX(destX);
	float newY = getRelativeY(destY);

	if (img) img->alphaFrameRender(hdc, newX, newY, frameX, frameY, alpha);
}
